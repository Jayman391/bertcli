...
class HeatMaps(object):
    ...
    @classmethod
    def generate_heatmap_from_df(
        cls, df: pd.DataFrame, xcol: str, ycol: str, weight_col: str = None, **kwargs
    ):
        """
    ...
    @classmethod
    def generate_marginal_histogram(
        cls,
        df: pd.DataFrame,
        col: str,
        weight_col: str,
        ...
        **kwargs,
        ...
        ax.hist(df[col], bins=bins, weights=weights, color=color, ec=color, **kwargs)
    ...
    def generate_ax_heatmap_hist(
        ...
        if gspec is not None:  # fig must be specified!
            try:
                ...
                msg = "Figure instance must be specified when `gspec` " "is `None`."
                logging.error(msg)
                raise Exception(msg)
...
def plot_2dhist(
    ...
    im = HeatMaps.generate_heatmap_from_df(data, ax=ax_heatmap, **heatmap_kwargs)
    ...
    ax_histy = HeatMaps.generate_marginal_histogram(
        data, ycol, weight_col=weight_col, ax=ax_histy, orientation="horizontal"
    )
...
def grid_2dhist(
    ...
    if gspec is None:
        gs_major = gridspec.GridSpec(maj_shape[0], maj_shape[1], **gs_major_kwargs)
    else:
        gs_major = gridspec.GridSpecFromSubplotSpec(
            maj_shape[0], maj_shape[1], subplot_spec=gspec, **gs_major_kwargs
        )

    gs_table = gridspec.GridSpecFromSubplotSpec(
        nrows, ncols, wspace=0.4, hspace=0.3, subplot_spec=gs_major[gs_maj_idx]
    )
    ...
    for nrow, ncol in itertools.product(range(nrows), range(ncols)):
        ...
        HeatMaps.generate_marginal_histogram(
            df, ycol, weight_col, ax=ax_histy, orientation="horizontal"
        )
    ...
    gs_colorbar = gridspec.GridSpecFromSubplotSpec(
        *gs_cbar_shape, subplot_spec=gs_major[gs_parent_cbar_idx], **gs_space
    )
    ax_colorbar = {}
    gs_internal_kwargs = dict(
        height_ratios=[0.35, 1.65],  # optimized height_ratios
    ...
    for n_bar in range(num_cbars):
        if group_cbar == "row":
            gs_colorbar_sub = gridspec.GridSpecFromSubplotSpec(
                2,
                1,
                subplot_spec=gs_colorbar[n_bar],
    ...
    if group_cbar == "row":
        ...
        for ncol in range(ncols):
            cb = fig.colorbar(
                im_dict[(0, ncol)], cax=ax_colorbar[ncol], orientation="horizontal"
            )
