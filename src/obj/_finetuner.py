...
class FineTuner:
    ...
    def load_model(self, model_filepath):
        ...
        model = LlamaForCausalLM.from_pretrained(
            model_filepath, device_map="auto", torch_dtype=torch.float16
        )
    ...
    def split_data(self, data_filepath, train_test_split_ratio=0.5):
        ...
        dataset = dataset["train"].train_test_split(
            test_size=train_test_split_ratio, seed=42, shuffle=True
        )
        return dataset
    ...
    def generate_and_tokenize_prompt(
        self, prompt, tokenizer, formatting_func, format_str
    ):
        ...
        if self.enable_profiler:
            ...
            profiler = torch.profiler.profile(
                ...
                on_trace_ready=torch.profiler.tensorboard_trace_handler(
                    f"{self.output_path}/logs/tensorboard"
                ),
                record_shapes=True,
                profile_memory=True,
                with_stack=True,
    ...
    def run(self):
        ...
        dataset = self.split_data(
            self.data_filepath, self.training_params["train_test_split_ratio"]
        )
        tokenized_train_dataset = dataset["train"].map(
            self.generate_and_tokenize_prompt
        )
        tokenized_validate_dataset = dataset["test"].map(
            self.generate_and_tokenize_prompt
        )
        ...
        model = self.train_model(
            ...
            tokenizer,
            config,
            self.training_params["total_steps"],
        )
